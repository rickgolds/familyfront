...
export class DashboardComponent implements OnInit {
  ...
  // Pobierz wszystkie listy zadań użytkownika i znajdź "Szybkie zadania"
  getTaskLists() {
    ...
    this.authService.getTaskLists().subscribe({
      next: (taskLists: TaskList[]) => {
        ...
        if (taskLists && taskLists.length > 0) {
          ...
          // Sprawdź, czy istnieje lista "Szybkie zadania"
  ...
  // Utwórz listę "Szybkie zadania", jeśli nie istnieje
  createQuickTasksList() {
    this.authService.addTaskList('Szybkie zadania').subscribe({
      next: (response) => {
        ...
        // Po utworzeniu pobierz listy ponownie, aby uzyskać ID nowej listy
        this.authService.getTaskLists().subscribe({
          next: (taskLists: TaskList[]) => {
            this.taskLists = taskLists;
            const quickTasksList = taskLists.find(
  ...
  // Pobierz zadania ze wszystkich list i posortuj je
