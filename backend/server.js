...
app.post("/tasks", authenticateToken, (req, res) => {
  const {title, description, assigned_to, list_id, due_date} = req.body;
  const created_by = req.user.id;
  ...
  function insertTask() {
    db.query(
      "INSERT INTO tasks (title, description, assigned_to, created_by, list_id, due_date) VALUES (?, ?, ?, ?, ?, ?)",
      [title, description, assigned_to, created_by, list_id, due_date || null],
      (err) => {
        if (err) {
...
app.put("/tasks/:id", authenticateToken, (req, res) => {
  ...
  db.query(
    ...
    (err, results) => {
      ...
      db.query(
        "UPDATE tasks SET title = ?, description = ?, status = ?, assigned_to = ?, due_date = ? WHERE id = ?",
        [title, description, status, assigned_to, due_date || null, taskId],
        (err) => {
          if (err) {
            console.error(
